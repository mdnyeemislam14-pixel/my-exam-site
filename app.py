...
# Safe import for autorefresh library
try:
    ...
    if admin_menu == "📊 অ্যাডমিন ড্যাশবোর্ড ও প্রশ্ন ম্যানেজমেন্ট":
        ...
        if not q_df.empty and "Subject" in q_df.columns:
            ...
            for idx, row in filtered_df.iterrows():
                with st.expander(f"প্রশ্ন {idx+1}: {str(row['Question'])[:60]}... (বিষয়: {row['Subject']})"):
                    with st.form(key=f"edit_form_{idx}"):
                        ...
                        if update_btn:
                            ...
                            st.success("✅ প্রশ্ন সফলভাবে আপডেট করা হয়েছে!")
                            st.rerun()

                        if delete_btn:
                            q_df = q_df.drop(idx).reset_index(drop=True)
                            q_df.to_csv(QUESTIONS_FILE, index=False)
                            st.success("🗑️ প্রশ্নটি সফলভাবে মুছে ফেলা হয়েছে!")
                            st.rerun()
        else:
        ...
        if input_mode == "টেক্সট পেস্ট (Easy Paste)":
            ...
            pasted_text = st.text_area(
                f"'{current_subject_key}'-এর সংরক্ষিত প্রশ্নসমূহ:",
            ...
            if st.button("📌 এই বিষয়ের প্রশ্ন সেভ/আপডেট করুন"):
                if not pasted_text:
                    ...
                    if parsed_questions:
                        ...
                        st.success(
                            f"✅ '{current_subject_key}' এর প্রশ্নসমূহ সফলভাবে আপডেট ও সেভ হয়েছে!"
                        )
                        st.rerun()
            ...
            if uploaded_file is not None:
                try:
                    ...
                    if end_idx <= len(raw_df.columns):
                        ...
                        st.info(f"🔍 এই কলাম রেঞ্জ থেকে মোট প্রশ্ন পাওয়া গেছে: {total_found}টি")

                        if st.button("📌 ফাইল থেকে এই বিষয়ের প্রশ্ন সেভ করুন", key="admin_save_from_file_btn"):
                            ...
                            if os.path.exists(CONFIG_FILE):
                                ...
                                final_conf = config_df
                            final_conf.to_csv(CONFIG_FILE, index=False)
                            st.success(f"✅ '{current_subject_key}' এর প্রশ্ন সফলভাবে সেভ হয়েছে!")
                            st.rerun()
                except Exception as e:
                    st.error(f"ত্রুটি: {e}")

else:
    # --- স্টুডেন্ট প্যানেল ও লাইভ এক্সাম পেজ ---
    all_q_df = (
        pd.read_csv(QUESTIONS_FILE) if os.path.exists(QUESTIONS_FILE) else pd.DataFrame()
    )
    active_subjects = (
        ...
        if student_menu == "🏆 ক্লাসের মেধা তালিকা (Leaderboard)":
            ...
            if not st.session_state["exam_in_progress"]:
                ...
                selected_subject = st.session_state.get(
                    "selected_exam_subject", active_subjects[0] if active_subjects else ""
                )
